...
2.数据库设计
    ...
    3)分析实体之间的关系
        用户登录信息 : 用户详细信息     1 : 1 PK(primary key)
        用户 : 日志                   1 : N
        日志 : 回复                   1 : N
        回复 : 主人回复                1 : 1 UK(unique key)
        用户 : 好友                   M : N

3.数据库范式:
    1)第一范式 : 列不可再分
    2)第二范式 : 一张表只表达一层含义(只描述一件事情)
    3)第三范式 : 表中的每一列和主键都是依赖关系，而不是间接依赖
    数据库性能和数据库规范度，要择重选择
4.数据库设计的范式和数据库查询性能很多时候是相悖的，我们需要根据实际的业务情况做一个选择:
    查询频次不高的情况下，我们倾向于提高数据库的设计范式，从而提高存储效率
    查询频次较高的情况下，我们倾向于牺牲数据库规范度，降低数据库设计范式，允许特定冗余，从而提高查询性能
